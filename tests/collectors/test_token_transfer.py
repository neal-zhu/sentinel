...
async def test_get_token(token_transfer_collector, mock_web3):
    ...
    with patch("sentinel.collectors.token_transfer.AsyncERC20Token") as mock_erc20:
        ...
        token = await token_transfer_collector._get_token(
        ...
        assert (
            "0x1234567890123456789012345678901234567890".lower()
            in token_transfer_collector.token_cache["ethereum"]
        )

        # Test getting same token again (from cache)
        cached_token = await token_transfer_collector._get_token(
            "ethereum", "0x1234567890123456789012345678901234567890"
        )
...
async def test_events_generator(token_transfer_collector, mock_web3):
    ...
    token_transfer_collector._scan_erc20_transfers = AsyncMock(
        return_value=[erc20_event]
    )
    token_transfer_collector._scan_native_transfers = AsyncMock(
        return_value=[native_event]
    )
    ...
    # 设置mock
    token_transfer_collector._scan_erc20_transfers = AsyncMock(
        side_effect=mock_erc20_generator
    )
    token_transfer_collector._scan_native_transfers = AsyncMock(
        side_effect=mock_native_generator
    )
    ...
    token_transfer_collector._scan_erc20_transfers.assert_called_once_with(
        "ethereum", 1000001, 1000010
    )
    token_transfer_collector._scan_native_transfers.assert_called_once_with(
        "ethereum", 1000001, 1000010
    )
...
async def test_persistent_storage():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        ethereum_config = {
            "ethereum": {"chain_id": 1, "rpc_endpoints": ["https://eth.example.com"]}
        }
        ...
        with patch("sentinel.collectors.token_transfer.AsyncWeb3"):
            with patch("sentinel.collectors.token_transfer.AsyncMultiNodeProvider"):
                ...
                collector1.state_store.store_collector_stats(
                    f"{component_name}:ethereum", stats
                )
        ...
        with patch("sentinel.collectors.token_transfer.AsyncWeb3"):
            with patch("sentinel.collectors.token_transfer.AsyncMultiNodeProvider"):
                with patch(
                    "sentinel.core.storage.BlockchainStateStore.get_last_processed_block",
                    side_effect=mock_get_block,
                ):
                    with patch(
                        "sentinel.core.storage.BlockchainStateStore.get_collector_stats",
                        ...
                        last_block = collector2.state_store.get_last_processed_block(
                            block_key
                        )
                        collector2.last_checked_block = {
                            "ethereum": last_block
                        ...
                        stats = collector2.state_store.get_collector_stats(
...
async def test_scan_erc20_transfers(token_transfer_collector):
    ...
    token_transfer_collector.web3_connections["ethereum"].keccak = MagicMock(
        return_value=bytes.fromhex(event_signature_hash[2:])
    )

    # Mock the eth.get_logs method
    token_transfer_collector.web3_connections["ethereum"].eth.get_logs = AsyncMock(
    ...
    token_transfer_collector.web3_connections[
        "ethereum"
    ].to_checksum_address = MagicMock(side_effect=convert_address)

    # Mock the block retrieval
    mock_block = {"timestamp": 1636000000}
    token_transfer_collector.web3_connections["ethereum"].eth.get_block = AsyncMock(
    ...
    # 收集异步生成器生成的所有事件
    transfers = []
    async for transfer in token_transfer_collector._scan_erc20_transfers(
        "ethereum", start_block, end_block
    ):
    ...
    token_transfer_collector.web3_connections[
        "ethereum"
    ].eth.get_logs.assert_called_once()
...
async def test_scan_native_transfers(token_transfer_collector):
    ...
    token_transfer_collector.web3_connections["ethereum"].eth.get_block = AsyncMock(
        side_effect=side_effect_get_block
    )
    ...
    # 收集异步生成器生成的所有事件
    transfers = []
    async for transfer in token_transfer_collector._scan_native_transfers(
        "ethereum", start_block, end_block
    ):
        transfers.append(transfer)
    ...
    assert (
        token_transfer_collector.web3_connections["ethereum"].eth.get_block.call_count
        == 1
    )

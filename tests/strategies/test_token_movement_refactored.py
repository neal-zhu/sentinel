    ...
    strategy = TokenMovementStrategy(
        ...
        config={
            "detectors": {
                ...
                "watched_address": {"addresses": ["0xWatched1", "0xWatched2"]},
                "watched_token": {
...
async def test_strategy_initialization(token_movement_strategy):
    ...
    assert detector_configs.get("significant_transfer", {}).get(
        "significant_transfer_threshold"
    ) == {"TEST": 50.0, "ETH": 1.0}

    # Check that the watch lists are set correctly
    assert detector_configs.get("watched_address", {}).get("addresses") == [
        "0xWatched1",
        "0xWatched2",
    ]
    assert detector_configs.get("watched_token", {}).get("tokens") == [
...
async def test_watched_address_detection(token_movement_strategy, token_transfer_event):
    ...
    assert (
        "0xWatched1" in addresses
    ), "Test setup issue: watched address not in configuration"

    # Process the event
    alerts = await token_movement_strategy.analyze_event(watched_event)

    # Debug: Print all alert titles
    for alert in alerts:
        print(f"DEBUG - Alert title: '{alert.title}'")

    # Check that an alert was generated
    assert len(alerts) > 0
    assert any("watched address" in alert.title.lower() for alert in alerts)
...
async def test_watched_token_detection(token_movement_strategy, token_transfer_event):
    ...
    assert (
        token_transfer_event.token_address in tokens
    ), "Test setup issue: watched token not in configuration"
    ...
    # Debug: Print all alert titles
    for alert in alerts:
        print(f"DEBUG - Token Alert title: '{alert.title}'")

    # Check that an alert was generated
    assert len(alerts) > 0
    assert any("watched token activity" in alert.title.lower() for alert in alerts)


@pytest.mark.asyncio
async def test_detector_integration(
    token_movement_strategy, token_transfer_event, mock_detector
):
...
async def test_filter_integration(
    token_movement_strategy, token_transfer_event, mock_filter
):
...
async def test_statistics_tracking(token_movement_strategy, token_transfer_event):
    ...
    assert (
        token_movement_strategy.token_stats[token_key]["total_volume"]
        == token_transfer_event.formatted_value
    )

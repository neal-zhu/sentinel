...
class TransactionCollector(Collector):
    ...
    async def _start(self):
        ...
        logger.info(
            f"Initialized TransactionCollector at block {self.last_processed_block}"
        )
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise
                logger.warning(
                    f"Failed to get latest block number (attempt {attempt + 1}): {e}"
                )
                await asyncio.sleep(self.retry_interval)
    ...
    async def _process_new_blocks(self) -> AsyncGenerator[TransactionEvent, None]:
        ...
        end_block = min(latest_block, start_block + self.max_blocks_per_batch - 1)
    ...
    async def _get_block_with_retry(self, block_number: int) -> Optional[BlockData]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    logger.error(
                        f"Failed to get block {block_number} after {self.max_retries} attempts: {e}"
                    )
                    return None
                logger.warning(
                    f"Failed to get block {block_number} (attempt {attempt + 1}): {e}"
                )
                await asyncio.sleep(self.retry_interval)

...
from ..core.events import Event, TransactionEvent
...
class GasTracker(Strategy):
    ...
    def __init__(self, windows: Dict[str, int] = None):
        ...
        self.gas_usage = defaultdict(
            lambda: defaultdict(list)
        )  # window -> contract -> [(timestamp, gas)]
    ...
    async def _get_contract_name(self, address: str) -> str:
        ...
        try:
            ...
            if contract_info and contract_info[0].get("Implementation"):
                ...
                impl_info = await self.etherscan.contract.contract_source_code(
                    impl_address
                )
    ...
    async def process_event(self, event: Event) -> List[Action]:
        ...
        if (
            ...
            actions.append(Action(type="gas_report", data=report))
    ...
    def _get_top_contracts(
        self, window: str, current_time: float
        ...
        for contract, usage_data in self.gas_usage[window].items():
            ...
            if total_gas > 0:
                ...
                change_rate = (
                    ((recent_gas / 300) / (old_gas / 300) - 1) * 100
                    if old_gas > 0
                    else 0
                )
    ...
    async def _generate_report(self, current_time: datetime) -> Dict:
        ...
        report = {"timestamp": current_time.isoformat(), "top_contracts": {}}

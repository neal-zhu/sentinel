...
class SmallTransferFilter(BaseFilter):
    ...
    def should_filter(self, event: TokenTransferEvent, context: Dict[str, Any]) -> bool:
        ...
        if (
            stats
            and "avg_transfer" in stats
            and stats.get("transfer_count", 0) > self.min_stats_count
        ):
            ...
            if event.formatted_value < (avg_transfer * self.small_transfer_threshold):
                logger.debug(
                    f"Filtering small transfer: {event.formatted_value} {event.token_symbol or 'tokens'} (avg: {avg_transfer})"
                )
                return True

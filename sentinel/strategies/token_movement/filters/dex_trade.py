...
class DexTradeFilter(BaseFilter):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.always_include_high_interest = self.config.get('always_include_high_interest', True)
    ...
    def is_likely_dex_trade(self, event: TokenTransferEvent, context: Dict[str, Any]) -> bool:
        ...
        if (AddressUtils.is_whitelisted_address(event.chain_id, event.from_address, whitelist_addresses) or
            AddressUtils.is_whitelisted_address(event.chain_id, event.to_address, whitelist_addresses) or
            AddressUtils.is_contract_address(event.from_address, known_dexes) or 
        ...
        if event.has_contract_interaction:
            ...
            if event.token_symbol and TokenUtils.is_common_dex_token(event.token_symbol):
                return True
        ...
        is_common_amount = any(abs(value - amt) / max(amt, 0.001) < 0.05 for amt in common_swap_amounts)
        ...
        if event.token_address:
            is_high_interest_token = TokenUtils.is_high_interest_token(event.chain_id, event.token_address)
        ...
        return (is_common_token and (is_round_number or is_common_amount)) or (is_round_number and is_common_amount)

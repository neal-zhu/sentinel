...
class PeriodicTransferDetector(BaseDetector):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.max_variation = self.config.get('max_variation', 0.25)  # 25% variation allowed
    ...
    async def detect(self, event: TokenTransferEvent, context: Dict[str, Any]) -> List[Alert]:
        ...
        outgoing_transfers = [
            t for t in transfers_by_address.get(address_key, [])
        ...
        recent_transfers = [t for t in outgoing_transfers if t.block_timestamp >= recent_time]
        ...
        try:
            ...
            variation = stdev_interval / avg_interval if avg_interval > 0 else float('inf')
            ...
            if variation <= self.max_variation:
                ...
                avg_interval_hours = ChainInfo.estimate_time_from_blocks(event.chain_id, avg_interval) / 3600
                ...
                frequent_recipients = [addr for addr, count in recipient_counts.items() if count >= min_count]
                ...
                if frequent_recipients:
                    recipient_info = f" to {len(frequent_recipients)} frequent recipients"
                    
                logger.info(f"Periodic transfer pattern detected for address {address}: ~{avg_interval_hours:.1f} hours interval")

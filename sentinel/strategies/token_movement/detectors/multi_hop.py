...
class MultiHopDetector(BaseDetector):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.arbitrage_time_window = self.config.get(
            "arbitrage_time_window", 60
        )  # seconds
    ...
    async def detect(
        ...
        from_transfers = [
            t
            for t in transfers_by_address.get(from_key, [])
            if t.block_timestamp >= window_start
        ]
        to_transfers = [
            t
            for t in transfers_by_address.get(to_key, [])
            if t.block_timestamp >= window_start
        ]
        ...
        if len(addresses_involved) >= self.min_addresses:
            ...
            for addr in addresses_involved:
                ...
                as_sender = sum(
                    1 for t in all_transfers if t.from_address.lower() == addr
                )
                as_receiver = sum(
                    1 for t in all_transfers if t.to_address.lower() == addr
                )
            ...
            if has_circular:
                ...
                if (
                    len(tokens_involved) >= self.min_tokens
                    ...
                    logger.info(
                        f"Multi-hop transfer pattern detected: {len(addresses_involved)} addresses, {len(tokens_involved)} tokens"
                    )

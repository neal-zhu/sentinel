...
class SignificantTransferDetector(BaseDetector):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.significant_transfer_threshold = self.config.get('significant_transfer_threshold', {})
    ...
    def is_significant_transfer(self, event: TokenTransferEvent, context: Dict[str, Any]) -> bool:
        """
        ...
        if chain_str not in self.significant_transfer_threshold:
            ...
            if TokenUtils.is_stablecoin(event.chain_id, event.token_address or '', event.token_symbol):
                return event.formatted_value >= (self.stablecoin_threshold * threshold_multiplier)
            else:
                return event.formatted_value >= (self.default_threshold * threshold_multiplier)

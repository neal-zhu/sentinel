...
class HighFrequencyDetector(BaseDetector):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.threshold = self.config.get(
            "threshold", 20
        )  # Default: 20 transfers per window
    ...
    async def detect(
        ...
        high_frequency_window_blocks = ChainInfo.estimate_blocks_from_time(
            chain_id, window_seconds
        )
        ...
        address_transfers = sum(
            count
            for blk, count in address_transfers_by_block.get(address_key, {}).items()
        ...
        if address_transfers >= self.threshold:
            ...
            logger.info(
                f"High-frequency trading detected for address {event.from_address}: {address_transfers} transfers in {high_frequency_window_blocks} blocks"
            )

...
class WashTradingDetector(BaseDetector):
    ...
    async def detect(self, event: TokenTransferEvent, context: Dict[str, Any]) -> List[Alert]:
        ...
        for t in from_transfers:
            if t.block_timestamp >= recent_time and t.to_address.lower() == event.to_address.lower():
                back_and_forth += 1
                
        for t in to_transfers:
            if t.block_timestamp >= recent_time and t.to_address.lower() == event.from_address.lower():
                back_and_forth += 1
        ...
        if back_and_forth >= self.back_and_forth_threshold:
            logger.info(f"Potential wash trading detected: {back_and_forth} transfers back and forth between {event.from_address} and {event.to_address}")

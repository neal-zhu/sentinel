...
class ContinuousFlowDetector(BaseDetector):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.flow_ratio_threshold = self.config.get(
    ...
    async def detect(
        ...
        for transfer in address_transfers:
            ...
            if (event.block_timestamp - transfer.block_timestamp).total_seconds() > (
        ...
        if abs(flow_ratio) >= self.flow_ratio_threshold:
            ...
            if pattern_type == "short_term_consecutive":
                ...
                description = (
                    f"Address {address} shows {recent_count} consecutive "
                ...
                description = (
                    f"Address {address} shows consistent {flow_type.lower()} pattern "
            ...
            logger.info(
                f"Continuous {flow_type.lower()} detected for address {address}: pattern={pattern_type}, net_flow={abs(net_flow)}"
            )

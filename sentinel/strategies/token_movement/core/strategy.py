...
from sentinel.strategies.token_movement.filters.simple_transfer import (
    SimpleTransferFilter,
)
from sentinel.strategies.token_movement.filters.small_transfer import (
    SmallTransferFilter,
)
...
class TokenMovementStrategy(Strategy):
    ...
    def __init__(
        ...
        logger.info(
            f"TokenMovementStrategy initialized for chain {self.chain_name} (ID: {self.chain_id})"
        )
    ...
    def _initialize_data_structures(self):
        ...
        self.transfers_by_address: Dict[str, List[TokenTransferEvent]] = defaultdict(
            list
        )

        # Track transfer frequencies by block window
        self.network_transfers_by_block: Dict[int, int] = defaultdict(int)
        self.address_transfers_by_block: Dict[str, Dict[int, int]] = defaultdict(
        ...
        self.alert_cooldown = self.config.get("strategy", {}).get(
            "alert_cooldown", 300
        )  # Default: 5 minutes
    ...
    def _initialize_plugins(self):
        ...
        try:
            ...
            logger.info(
                f"TokenMovementStrategy initialized with {len(self.detectors)} detectors and {len(self.filters)} filters"
            )
    ...
    def _update_statistics(self, event: TokenTransferEvent):
        ...
        if event.chain_id != self.chain_id:
            logger.warning(
                f"Received event from chain {event.chain_id}, but this strategy handles chain {self.chain_id}"
            )
            return
        ...
        for address in [event.from_address, event.to_address]:
            ...
            if address not in self.address_stats:
                self.address_stats[address] = {
                    ...
                    "interacted_with": {
                        event.to_address if is_sender else event.from_address
                    },
                }
            else:
                # Update existing stats
    ...
    def _deduplicate_alerts(self, alerts: List[Alert]) -> List[Alert]:
        ...
        if deduplicated:
            ...
            for alert in deduplicated:
                alert_types_count[alert.title] = (
                    alert_types_count.get(alert.title, 0) + 1
                )
    ...
    async def _prepare_event(self, event: TokenTransferEvent) -> TokenTransferEvent:
        ...
        if not token_symbol:
            ...
            token_symbol = TokenUtils.get_token_symbol(
                event.chain_id, event.token_address or "", self.token_symbols_cache
            )
            logger.debug(f"Initialized token symbol: {token_symbol}")

        # Check if we need to initialize formatted value
        formatted_value = (
            event.formatted_value if hasattr(event, "formatted_value") else None
        )
        ...
        if needs_model_copy:
            try:
                ...
                try:
                    ...
                    logger.error(
                        "Unable to update event fields due to Pydantic model restrictions"
                    )
    ...
    async def process_event(self, event: Event) -> List[Any]:
        ...
        logger.debug(
            f"Processing token transfer: chain={modified_event.chain_id}, tx={modified_event.transaction_hash}, from={modified_event.from_address}, to={modified_event.to_address}"
        )
        ...
        if alerts:
            logger.info(
                f"Generated {len(alerts)} alerts for token transfer: chain={modified_event.chain_id}, tx={modified_event.transaction_hash}, value={modified_event.formatted_value} {modified_event.token_symbol}"
            )
    ...
    async def analyze_event(self, event: TokenTransferEvent) -> List[Alert]:
        ...
        watched_addresses = detector_configs.get("watched_address", {}).get(
            "addresses", []
        )
        watched_tokens = detector_configs.get("watched_token", {}).get("tokens", [])
        whitelist_addresses = filter_configs.get("whitelist", {}).get(
            "whitelist_addresses", []
        )
        ...
        for filter_name, filter_plugin in self.filters.items():
            if filter_plugin.is_enabled() and filter_plugin.should_filter(
                event, context
            ):
                logger.debug(
                    f"Event filtered by {filter_name}: {event.transaction_hash}"
                )
                should_filter = True
                break
                    ...
                    if detector_alerts:
                        for alert in detector_alerts:
                            ...
                            if (
                                hasattr(alert, "data")
                                and alert.data
                                and "pattern_type" in alert.data
                            ):
                            ...
                            if self._should_alert(alert_key):
                                ...
                                logger.debug(
                                    f"Alert from {detector_name} rate-limited: {alert_key}"
                                )
    ...
    async def process(self, events: List[Event]) -> List[Any]:
        ...
        logger.debug(
            f"TokenMovementStrategy.process: Processing batch of {len(events)} events"
        )
        ...
        for event in events:
            ...
            if isinstance(event, TokenTransferEvent):
                ...
                if alerts:
                    logger.debug(
                        f"Generated {len(alerts)} alerts for event {event.transaction_hash}"
                    )
        ...
        if all_alerts:
            logger.info(
                f"TokenMovementStrategy.process: Processed {token_transfer_count}/{len(events)} token transfers, generated {len(all_alerts)} alerts/actions"
            )
        else:
            logger.debug(
                f"TokenMovementStrategy.process: Processed {token_transfer_count}/{len(events)} token transfers, no alerts generated"
            )

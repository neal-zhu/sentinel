...
class MultiNodeProvider(HTTPProvider):
    ...
    def _get_available_provider(self) -> Optional[HTTPProvider]:
        ...
        for idx, provider in available_providers:
            ...
            if current_time - self.last_used[endpoint] >= (1.0 / self.rate_limit):
                ...
                if (
                    current_time - self.last_health_check[endpoint]
    ...
    def _check_provider_health(self, provider: HTTPProvider, endpoint: str) -> bool:
        ...
        try:
            ...
            if "result" in response:
                ...
                logger.warning(
                    f"Node health check failed for {endpoint}: No result in response"
                )
    ...
    async def _check_provider_health_async(
        self, provider: HTTPProvider, endpoint: str
    ) -> bool:
        ...
        while attempts < self.max_retries:
            ...
            try:
                ...
                if "error" in response:
                    ...
                    logger.warning(
                        f"Node {endpoint} returned error: {response['error']}"
                    )
                    # Only mark as unhealthy for serious errors
                    if "code" in response["error"] and response["error"]["code"] in [
                        -32000,
                        -32603,
                        -32002,
                    ]:
...
class AsyncMultiNodeProvider(AsyncHTTPProvider):
    ...
    async def make_request(
        self, method: RPCEndpoint, params: Sequence[Any]
    ) -> RPCResponse:
        """
    ...
    async def is_connected(self, show_traceback: bool = False) -> bool:
        ...
        """
        return await self.multi_provider.is_connected_async(

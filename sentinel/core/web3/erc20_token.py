    ...
    def get_transfer_events(
        ...
        def fill_address_filter(key: str, addresses: Optional[Union[str, List[str]]]):
            if isinstance(addresses, str):
                ...
                valid_addresses = [
                    Web3.to_checksum_address(addr) for addr in addresses 
        ...
        try:
            ...
            logger.error(f"Error getting logs directly, falling back to contract events API: {e}")
            return []
    ...
    async def get_transfer_events(
        ...
        def fill_address_filter(key: str, addresses: Optional[Union[str, List[str]]]):
            if isinstance(addresses, str):
                ...
                valid_addresses = [
                    self.web3.to_checksum_address(addr) for addr in addresses 
        ...
        try:
            ...
            for log in logs:
                ...
                event = {
                    ...
                    'address': log['address'],
                    'args': {
                        'from': self.web3.to_checksum_address('0x' + topics[1].hex()[-40:]),
                        'to': self.web3.to_checksum_address('0x' + topics[2].hex()[-40:]),
                        'value': value
                    }
                }
                events.append(event)

...
class StatsManager:
    ...
    async def start(self):
        ...
        self.running = True
        self._task = asyncio.create_task(self._log_stats(), name="stats_manager")
        return self._task
    ...
    async def _log_stats(self):
        ...
        while self.running:
            try:
                ...
                # Calculate rates
                event_collect_rate = (
                    self.events_collected / elapsed if elapsed > 0 else 0
                )
                event_process_rate = (
                    self.events_processed / elapsed if elapsed > 0 else 0
                )
                ...
                # Get queue sizes
                events_queued = (
                    self.get_collector_queue_size()
                    if self.get_collector_queue_size
                    else 0
                )
                actions_queued = (
                    self.get_executor_queue_size()
                    if self.get_executor_queue_size
                    else 0
                )
                ...
                if collector_idle > 60:  # 1 minute
                    logger.warning(
                        f"Collector has been idle for {collector_idle:.1f} seconds"
                    )
                if strategy_idle > 60:
                    logger.warning(
                        f"Strategy processor has been idle for {strategy_idle:.1f} seconds"
                    )
                if executor_idle > 60:
                    logger.warning(
                        f"Action executor has been idle for {executor_idle:.1f} seconds"
                    )
